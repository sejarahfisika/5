...
let challengeModeType = "challenge";
...
function showIntroStep1() {
  if (!introStep1 || !introStep2) return;
...
function completeIntro() {
  const name = introNameInput ? introNameInput.value.trim() : "";
  const nim = introNimInput ? introNimInput.value.trim() : "";
  const kelas = introClassInput ? introClassInput.value.trim() : "";
  const prodi = introStudyInput ? introStudyInput.value.trim() : "";
...
// overlay click -> tidak menutup modal
